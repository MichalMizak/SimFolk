...
public class LyricCleaner implements ILyricCleaner {
    ...
    private String cleanRepetition(String lyrics) {
        Set<String> repetitionStringsSet = getRepetitionStringsSet();
        for (String s : repetitionStringsSet) {
            lyrics = lyrics.replaceAll(s, "");
        }
        return lyrics;
    }

    private Set<String> getRepetitionStringsSet() {
        Set<String> repetitionStringsSet = new HashSet<>();

        String[] repetitionStrings = new String[]{"//.//.//.", "Ref", "Refr"};

        for (String specialCharacter : repetitionStrings) {
            repetitionStringsSet.add(specialCharacter);
        }

        return repetitionStringsSet;
    }
